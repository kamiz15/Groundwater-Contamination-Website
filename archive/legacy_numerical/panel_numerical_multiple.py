...
def _query_int(name, default=0):
    try:
        ...
        for candidate in PARAM_ALIASES.get(name, (name,)):
            if candidate in req.arguments:
                return int(float(req.arguments[candidate][0].decode()))
        return default
    except Exception:
        return default
...
def _run_row(row):
    ...
    hk       = float(row.get("hk [m/d]", 1.0))
    vk       = float(row.get("vk [m/d]", hk))
    C0       = float(row.get("C0",       8.0))
    perlen   = float(row.get("perlen",   100.0))
    Sw       = float(row.get("Sw",       5.0))
    R_Wu     = float(row.get("R_Wu",     7.5))
    R_Wb     = float(row.get("R_Wb",     7.5))
    alpha_th = float(row.get("alpha_th", 0.1))
    L_D_v_override = float(row.get("L_D_v_override [m]", 0.0))
    L_D_h_override = float(row.get("L_D_h_override [m]", 0.0))
    ...
    if min(prsity, al, av, hk, vk, C0, perlen) <= 0:
        raise ValueError("Porosity, dispersivity, hydraulic conductivities, C0, and simulation time must be > 0.")
    ...
    L_D_v = liedl_domain_length(analytical_lmax, L_D_v_override)
    ...
    v_result = run_numerical_model(
        L_D_v, A_T, n_cols_v, n_rows_v, prsity, al, av, gamma, Cd, Ca, h1, h2, hk, vk,
        source_thickness=S_T,
        source_bottom_buffer=R_Tb,
        perlen=perlen,
        plume_threshold=C0,
        ath=alpha_th,
    )
    ...
    L_D_h = cirpka_domain_length(cirpka_lmax_val, L_D_h_override)
    A_W = R_Wb + Sw + R_Wu
...
def numerical_multiple_app():
    default_df = pd.DataFrame([
        {
            ...
            "hk [m/d]": _query_float("hk",        1.0),
            "vk [m/d]": _query_float("vk",        _query_float("K_v", _query_float("hk", 1.0))),
            "C0":       _query_float("C0",        8.0),
            "perlen":   _query_float("perlen",    100.0),
            "Sw":       _query_float("Sw",        5.0),
            "R_Wu":     _query_float("R_Wu",      7.5),
            "R_Wb":     _query_float("R_Wb",      7.5),
            "alpha_th": _query_float("alpha_th",  0.1),
            "L_D_v_override [m]": _query_float("L_D_v_override", 0.0),
            "L_D_h_override [m]": _query_float("L_D_h_override", 0.0),
        }
    ])
    ...
    def _pdf_callback():
        if not _state:
            return io.BytesIO(b"")
        report = CASTReport("Numerical Model \u2014 Multiple Simulation", "Numerical MODFLOW 6 GWF/GWT")
        ...
        try:
            ...
            _state.update({
                ...
                "vertical_optional_view": {
                    "concentration": v_res.concentration,
                    "x_grid": v_res.x_grid,
                    "cross_grid": v_res.z_grid,
                    "cross_axis_label": "Aquifer Thickness [m]",
                    "title": "Vertical Numerical Model - Scenario 1",
                },
                "horizontal_optional_view": {
                    "concentration": h_res.concentration,
                    "x_grid": h_res.x_grid,
                    "cross_grid": h_res.y_grid,
