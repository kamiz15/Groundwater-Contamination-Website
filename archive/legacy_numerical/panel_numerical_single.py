...
from analytical_models import cirpka_lmax as _cirpka_lmax, cirpka_domain_length, liedl_domain_length, liedl_lmax
from data_queries import get_user_sites
from numerical_models import run_numerical_model, run_numerical_model_horizontal
from panel_analytical_common import PARAM_ALIASES
...
def _query_float(name, default):
    try:
        ...
        raw = None
        for candidate in PARAM_ALIASES.get(name, (name,)):
            if candidate in req.arguments:
                raw = req.arguments[candidate][0].decode()
                break
...
def _database_lmax_points(email, alpha_tv, gamma, selected_site_id=0):
    try:
        ...
        user_safe_error(exc, "Database error while loading numerical comparison points")
        return [], [], None
...
def numerical_single_app():
    ...
    hk       = pn.widgets.FloatInput(name="Horizontal Hydraulic Conductivity K_h [m/d]", value=_query_float("hk", 1.0),     step=0.1)
    vk       = pn.widgets.FloatInput(name="Vertical Hydraulic Conductivity K_v [m/d]", value=_query_float("vk", _query_float("K_v", hk.value)), step=0.1)
    c0       = pn.widgets.FloatInput(name="Plume Contour Threshold C0 [mg/L]",         value=_query_float("C0", 8.0),     step=0.1)
    perlen   = pn.widgets.FloatInput(name="Simulation Time [day]",                    value=_query_float("perlen", 100.0), step=1.0)
    ld_v_override = pn.widgets.FloatInput(name="Vertical Domain Length LD Override [m] (0 = automatic)", value=_query_float("L_D_v_override", 0.0), step=1.0)
    ld_h_override = pn.widgets.FloatInput(name="Horizontal Domain Length LD Override [m] (0 = automatic)", value=_query_float("L_D_h_override", 0.0), step=1.0)
    ...
    email            = _query_str("email", "")
    selected_site_id = _query_int("site_id", 0)

    _state: dict = {}
    vertical_optional_views = LazyNumericalViews(lambda: _state.get("vertical_optional_view"))
    horizontal_optional_views = LazyNumericalViews(lambda: _state.get("horizontal_optional_view"))

    def _pdf_callback():
        if not _state:
            return io.BytesIO(b"")
        report = CASTReport("Numerical Model \u2014 Single Simulation", "Numerical MODFLOW 6 GWF/GWT")
    ...
    def _run(_=None):
        _state.pop("vertical_optional_view", None)
        _state.pop("horizontal_optional_view", None)
        vertical_optional_views.reset()
        horizontal_optional_views.reset()
        ...
        try:
            ...
            if min(prsity.value, al.value, av.value, hk.value, vk.value, c0.value, perlen.value) <= 0:
                raise ValueError("Porosity, dispersivity, hydraulic conductivities, C0, and simulation time must be positive.")
            ...
            L_D_v = liedl_domain_length(analytical_lmax, ld_v_override.value)
            ...
            v_result = run_numerical_model(
                ...
                h1.value, h2.value, hk.value, vk.value,
                source_thickness=s_t.value,
                source_bottom_buffer=r_tb.value,
                perlen=perlen.value,
            ...
            L_D_h = cirpka_domain_length(cirpka_lmax_val, ld_h_override.value)
            ...
            plot_images = [
                {
                    ...
                    "caption": "Vertical numerical plume generated by the MODFLOW 6 GWF/GWT model.",
                },
                {
                    ...
                    "caption": "Horizontal numerical plume generated by the MODFLOW 6 GWF/GWT model.",
                },
                {
            ...
            _state.update({
                ...
                "vertical_optional_view": {
                    "concentration": v_result.concentration,
                    "x_grid": v_result.x_grid,
                    "cross_grid": v_result.z_grid,
                    "cross_axis_label": "Aquifer Thickness [m]",
                    "title": "Vertical Numerical Model",
                },
                "horizontal_optional_view": {
                    "concentration": h_result.concentration,
                    "x_grid": h_result.x_grid,
                    "cross_grid": h_result.y_grid,
            ...
            result_pane.object = _result_card("Error", f'<div style="font-size:0.98rem;color:#5f1d1d;">{safe_error}</div>', tone="error")
    ...
    transport_card = pn.Column(
        ...
        prsity, al, av, gamma, cd, ca, c0, h1, h2, hk, vk, perlen,
        sizing_mode="stretch_width",
        styles={**card, "flex": "1 1 320px", "min-width": "280px"},
    )
    ...
    inputs = pn.Column(
        pn.FlexBox(domain_card, transport_card, horiz_card,
                   sizing_mode="stretch_width", flex_wrap="wrap", styles={"gap": "14px"}),
        pn.Accordion(
            ("Advanced Overrides", pn.Column(ld_v_override, ld_h_override, sizing_mode="stretch_width")),
            active=[],
        ),
        sizing_mode="stretch_width",
    )
    ...
    optional_section = pn.Tabs(
        ("Vertical Optional Views", vertical_optional_views.panel),
        ("Horizontal Optional Views", horizontal_optional_views.panel),
        sizing_mode="stretch_width",
    )
