...
def liedl3d_single_app():
    ...
    cthres = pn.widgets.FloatInput(name="Threshold Donor Concentration C_thres [mg/L]", value=query_float("Cthres", 0.5), step=0.01)
    ...
    gamma = pn.widgets.FloatInput(name="Stoichiometry Ratio \u03b3 [-]", value=query_float("gamma", 3.0), step=0.1)
        ...
        try:
            ...
            _state.update({
                "parameters": [
                    ...
                    {"symbol": "C_thres", "name": "Threshold Donor Concentration", "value": cthres.value, "unit": "mg/L"},
