...
class MultipleStageLayoutTests(MultipleWrapperTests):
    ...
    # Liedl opts out: its Panel owns an editable scenario table, and this form
    # reloads the page on submit.
    ANALYTICAL = [p for p in MULTIPLE_WRAPPERS
                  if not p.startswith("/numerical/") and p != "/liedl/multiple"]
    ...
    def test_a_page_can_opt_out_of_the_form(self):
        page = self.client.get("/liedl/multiple").get_data(as_text=True)

        self.assertIn("model-workbench-stage--multiple", page)   # same stage
        self.assertNotIn("model-input-form", page)               # without the form
    ...
    def test_running_the_model_keeps_the_picked_sites(self):
        ...
        with patch.object(analytical_routes, "get_user_sites_rows", return_value=sites), \
            ...
            page = self.client.get("/chu/multiple?compare_sites=2").get_data(as_text=True)
        ...
        self.assertIn('name="alpha_Th"', sidebar)
