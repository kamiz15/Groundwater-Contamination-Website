...
def birla_single_app():
    title = pn.pane.Markdown("## Birla et al. - Single Simulation")

    w_M = pn.widgets.FloatInput(name="Aquifer thickness M", value=_query_float("M", 2.0), step=0.1)
    w_tv = pn.widgets.FloatInput(name="Vertical transverse dispersivity tv", value=_query_float("tv", 0.001), step=0.0005)
    ...
    w_R = pn.widgets.FloatInput(name="Recharge rate R", value=_query_float("R", 1.0), step=0.1)
